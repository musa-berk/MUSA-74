...
\begin{proof}
    ...
    \end{enumerate}
\end{proof}
Here is a more conceptual inductive result, which we will use in the proof of \Cref{prop:finite-inj-surj-bij}.
...
\begin{proof}
    ...
    \end{enumerate}
\end{proof}
% And here is another set-theoretic example.
% \begin{prop}
%     Let $X$ be a finite set with $n$ elements, where $n\in\NN$. Then $\mathcal P(X)$ has $2^n$ elements.
% \end{prop}
% \begin{proof}
%     We are unable to be exWe proceed with induction on $n$.
%     \begin{enumerate}
%         \item Base case: when $n=0$, we see $X=\emp$, so $\mathcal P(X)=\{\emp\}$, so $\mathcal P(X)$ has $1=2^0$ elements.
%         \item Inductive step: suppose the statement is true for all $n$-element sets $X$. We show that it is true for a given $(n+1)$-element set $X'$.

%         Now, list out the elements of $X'$ by $\{x_1,\ldots,x_{n+1}\}$. Then define $X\coloneqq\{x_1,\ldots,x_n\}$ to be an $n$-element subset of $X'$. 
%     \end{enumerate}
% \end{proof}
...
\end{homework}
\begin{homework}
    Using induction and \Cref{prop:de-morgan}, prove the following.
