...
\end{restatable}
\noindent \Cref{thm:groups-of-ord-p} is really amazing: given only knowledge about the size of $G$, we are immediately able to make deductions about the group structure of $G$. This result is one example of what we mean when we say ``groups have structure'': a small amount of information leads to a larger amount of information because of ``structural'' constraints.

Quickly, note that we have dealt with one-element groups in \Cref{ex:groups-of-ord-1}, so \Cref{thm:groups-of-ord-p} is the next simplest ``classification'' result for groups. Further, observe that groups can in general be somewhat complicated. For example, there are non-isomorphic groups of size $4$.
\begin{example}
    Let $V$ denote the subgroup $\left\{e,s,r^2,sr^2\right\}$ of $(D_4,\cdot)$. (See \Cref{exe:klein-four}.) We claim that $V$ and $\ZZ/4\ZZ$ are not isomorphic. To begin, note that $g^2=e$ for all $g\in V$, which we can check directly: note $s^2=r^4=e$. Now, let $\varphi\colon\ZZ/4\ZZ\to V$ be any homomorphism, and we show that $\varphi$ is not an isomorphism. Indeed, note
...
\end{lemma}
\begin{proof}
    The key input here is \Cref{thm:lagrange}. Indeed, by \Cref{thm:lagrange}, either $\left|H\right|=1$ or $\left|H\right|=p$ because $1$ and $p$ are the only positive divisors of $p$. We thus have two cases.
    \begin{itemize}
...
\groupsofordp*
\begin{proof}
    Because $\left|G\right|>1$, we may select an element $g\in G$ not equal to the identity $e\in G$. By \Cref{prop:ord-of-g-divides-ord-of-g}, note that $g^p=e$, where $e\in G$ is the identity. For intuition, note that the proof of \Cref{prop:ord-of-g-divides-ord-of-g} shows that
    ...
    \begin{itemize}
        \item Well-defined: suppose $[k]_p=[\ell]_p$, and we want to show that $g^k=g^\ell$. Well, $[k]_p=[\ell]_p$ implies that $p\mid k-\ell$, so we may use \Cref{thm:division} to write $k=pq+\ell$ for some integer $q$. Then
        \[g^k=g^{pq+\ell}=\left(g^p\right)^q\cdot g^\ell=e^q\cdot g^\ell=g^\ell.\]
        \item Injective: by \Cref{lem:ker-subgroup}, it is enough to show that $\ker\varphi=\{[0]_p\}$. Well, we know that $\ker\varphi$ is a subgroup of $\ZZ/p\ZZ$ already, so \Cref{lem:subgroups-of-prime-order-group} implies that $\ker\varphi=\{[0]\}_p$ or $\ker\varphi=\ZZ/p\ZZ$. However, $\varphi([1]_p)=g\ne e$ by definition, so $[1]_p\notin\ker\varphi$, so $\ker\varphi\ne\ZZ/p\ZZ$, so $\ker\varphi=\{[0]_p\}$ follows.
        \item Surjective: by \Cref{lem:im-subgroup}, it is enough to show that $\im\varphi=G$. Well, we know that $\im\varphi$ is a subgroup of $G$ already, so \Cref{lem:subgroups-of-prime-order-group} implies that $\im\varphi=\{e\}$ or $\im\varphi=G$. Well, $g=\varphi([1]_p)\in\im\varphi$, so $\im\varphi\ne\{e\}$, so $\im\varphi=G$ follows.
        \qedhere
    \end{itemize}
\end{proof}
